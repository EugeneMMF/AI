...
class Dense(BaseLayer):
    def __init__(self, nodes, input_shape = None, activation = None, parameters = {}):
        ...
        if input_shape:
            self.weights = np.random.uniform(-1./np.sqrt(self.input_shape[0]), 1./np.sqrt(self.input_shape[0]), (self.nodes,input_shape[0]))
        else:
            self.weights = None
    ...
    def backward(self, error, learning_rate, optimizer = None, parameters = {}):
        ...
        self.input_error = np.dot(self.weights.T, unaltered_error)
        self.weights -= learning_rate * np.dot(error, self.input.T)
    ...
    def get_errors(self, error,optimizer = None, parameters = {}):
        ...
        if optimizer == momentum:
            ...
            raise Exception("Invalid optimizer")
        input_error = np.dot(self.weights.T, unaltered_error)
        weights_error = np.dot(error, self.input.T)
        biases_error = error
...
class Conv2D(BaseLayer):
    ...
    def compile(self, input_shape):
        ...
        self.activation = get_function(self.activation_name)
        verify_activation(self.activation, self.parameters)
        if not self.input_shape or input_shape != self.input_shape:
            raise Exception(f"Input shape from previous layer {input_shape} does not match declared input shape {self.input_shape}")
        self.input_shape = input_shape
        if self.padding == "same":
            self.output_shape = self.input_shape
        else:
            self.output_shape = ((self.input_shape[0] - self.kernel_size[0])/self.strides[0] + 1, (self.input_shape[1] - self.kernel_size[1])/self.strides[1] + 1, self.filters)
            ...
            self.output_shape = (int(self.output_shape[0]), int(self.output_shape[1]), int(self.output_shape[2]))
...
class Flatten(BaseLayer):
    ...
    def compile(self, input_shape):
        self.input_shape = input_shape
        self.output_shape = (np.product(input_shape), 1)
        return self.output_shape
